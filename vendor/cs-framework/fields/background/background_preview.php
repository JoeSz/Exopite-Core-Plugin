 ...
 *
 */
class CSFramework_Option_background_preview extends CSFramework_Options {
  ...
  public function output() {
    ...
    global $wpdb;
    $attachment = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE guid='%s';", $this->value['image'] ));
    $value = ( isset( $attachment[0] ) ) ? $attachment[0] : null;

    $preview = '';
    //$value   = pippin_get_image_id($this->value['image']);
    ...
    echo '<div class="cs-image-preview'. $hidden .'"><div class="cs-preview"><img src="'. $preview .'" alt="preview" /></div></div>';
